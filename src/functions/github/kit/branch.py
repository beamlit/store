...
async def search_code(gh: Github, **kwargs):
    ...
    class SearchCodeInput(RepositoryBranchInput):
        query: str = Field(description="A keyword-focused natural language search query for code, e.g. `MyFunctionName()`.")

    input = SearchCodeInput(**kwargs)
    search_results = gh.search_code(query=input.query, repo=input.repository, ref=input.branch)
    ...
    for code in search_results:
        ...
        file_content = repo.get_contents(code.path, ref=input.branch).decoded_content.decode()
        results.append(
...
async def create_branch(gh: Github, **kwargs):
    ...
    class CreateBranchInput(RepositoryInput):
        branch: str = Field(description="The name of the branch to create, e.g. `my_branch`")
    input = CreateBranchInput(**kwargs)
    repo = gh.get_repo(input.repository)
    repo.create_git_ref(ref=f"refs/heads/{input.branch}", sha=repo.get_branch("main").commit.sha)
...
async def delete_branch(gh: Github, **kwargs):
    ...
    class DeleteBranchInput(RepositoryInput):
        branch: str = Field(description="The name of the branch to delete, e.g. `my_branch` **IMPORTANT**: only send the name of the branch, not heads/my_branch")
