...
async def create_file(gh: Github, **kwargs):
    ...
    class CreateFileInput(RepositoryBranchInput):
        path: str = Field(
            description="The path to the file to be created, e.g. `src/my_file.py`"
        )
        content: str = Field(description="The content of the file to be created")
        message: str = Field(
            description="The commit message to be used for the file creation"
        )
...
async def update_file(gh: Github, **kwargs):
    ...
    class UpdateFileInput(RepositoryBranchInput):
        path: str = Field(
            description="The full file path of the file you would like to update where the path must NOT start with a slash, e.g. `some_dir/my_file.py`."
        )
        content: str = Field(description="The new content of the file to be updated")
        message: str = Field(
            description="The commit message to be used for the file update"
        )
    ...
    repo.update_file(
        input.path, input.message, input.content, file.sha, branch=input.branch
    )
...
async def delete_file(gh: Github, **kwargs):
    ...
    class DeleteFileInput(RepositoryBranchInput):
        path: str = Field(
            description="The full file path of the file you would like to delete where the path must NOT start with a slash, e.g. `some_dir/my_file.py`."
        )
...
async def list_files(gh: Github, **kwargs):
    ...
    class ListFilesInput(RepositoryBranchInput):
        path: str = Field(
            default="",
            description="The path to the directory to list the files from, e.g. `some_dir`",
        )
